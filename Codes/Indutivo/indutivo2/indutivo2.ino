...
#define pinVEL 19 // Pino de interrupção para rotação da roda
#define pinVEL2 18 // Pino de interrupção para rotação da roda
...
volatile byte pulsosVEL2 = 0; //contador de pulsos para velocidade
float VEL = 0; //velocidade em km/h
float VEL2 = 0; //velocidade em km/h
...
void tacometro() 
{
  ...
  //Serial.print(pulsosVEL);
}

void tacometro2() 
{
  pulsosVEL2++; // Função chamada no pino de interrupção
  //Serial.print(pulsosVEL);
}
...
void setup(){
  ...
  attachInterrupt(digitalPinToInterrupt(pinVEL), tacometro, RISING); //Interrupção para ler pulso da velocidade
   attachInterrupt(digitalPinToInterrupt(pinVEL2), tacometro2, FALLING); //Interrupção para ler pulso da velocidade
}
void loop(){
  velocidade();
}